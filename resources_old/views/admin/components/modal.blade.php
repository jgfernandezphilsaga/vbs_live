...
<script>
    $(document).ready(function () {
        $('#requestDetailModal').on('show.bs.modal', function(event) {
            ...
            $.ajax({
                url: '{{ env('APP_URL') }}/request/show/' + id,
                method: 'GET',
                success: function(response) {
                    ...
                    if (response.vehicle_types && response.vehicle_types.length > 0) {
                        let vehicleDisplay = response.vehicle_types
                            .map(v => `${v.vehicle_type} (${v.qty})`)
                            .join(', ');
                            
                        $("#requested-vehicle").html(vehicleDisplay);
                    } else {
    ...
    function updateRequest(id, status) {
        ...
        $.ajax({
            url: '{{ env('APP_URL') }}/request/update-status',
            method: 'POST',
            data: {
                id: id, 
                status: status,

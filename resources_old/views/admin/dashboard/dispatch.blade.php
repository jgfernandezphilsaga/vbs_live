            ...
            <div class="card">
                <div class="card-body row">
                    ...
                    <div class="row justify-content-evenly m-0">
                        ...
                        <div class="col-md-2 col-sm-6 card-container p-0">
                            <a target="_blank">
                                <div class="card counter-card" style=" background-color: #ff6f3c;" data-status="1011">
                                    ...
                                    <div class="card-body">
                                        <div class="p-0 m-0" style="display: flex; flex-direction:row; justify-content:space-between;">
                                            <h1 style="font-weight:bold; margin:0px; padding:0px;">{{ $pending }}</h1>
                        ...
                        <div class="col-md-2 col-sm-6 card-container p-0">
                            <div class="card dash-approved-card counter-card" data-status="1007">
                                <div class="card-title p-2 m-2">ASSIGN/DISPATCH</div>
                                <div class="card-body">
                                    <div class="p-0 m-0" style="display: flex; flex-direction:row; justify-content:space-between;">
                                        <h1 style="font-weight:bold; margin:0px; padding:0px;">{{ $dispatch }}</h3>
                        ...
                        <div class="col-md-2 col-sm-6 card-container p-0">
                            <div class="card counter-card" style="background-color: #e46161;" data-status="1009">
                                <div class="card-title p-2 m-2">APPROVED(OPEN)</div>
                                <div class="card-body">
                                    <div class="p-0 m-0" style="display: flex; flex-direction:row; justify-content:space-between;">
                                        <h1 style="font-weight:bold; margin:0px; padding:0px;">{{ $approved_open }}</h3>
                        ...
                        <div class="col-md-2 col-sm-6 card-container p-0">
                            <div class="card counter-card" style="background-color: #c24d2c;" data-status="1009">
                                <div class="card-title p-2 m-2">APPROVED(CLOSED)</div>
                                <div class="card-body">
                                    <div class="p-0 m-0" style="display: flex; flex-direction:row; justify-content:space-between;">
                                        <h1 style="font-weight:bold; margin:0px; padding:0px;">{{ $approved_closed }}</h3>
                                        <i class="fa-solid fa-list-check fa-5x card-icon"></i>
                                    </div>    
                                </div>
                            </div>
                        ...
                        <div class="d-flex flex-row">
                                <select class="form-select form-select-lg me-2" style="font-size: 12px; width: 10vw; min-width: 2vw;" id="filter-department" name="filter_department" aria-label="Large select example" style="font-size: 13px;">
                                    ...
                                    @foreach($departments as $dept)
                                        <option value="{{ $dept->requesting_dept }}">{{ $dept->requesting_dept }}</option>
                                    @endforeach
                                </select>
                    ...
                    <table data-order='[[ 0, "desc" ]]' id="dashboard-table" class="table table-hover table-bordered table-striped table-custom mb-0" style="border-top-left-radius: 8px; table-layout:fixed">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align:center; background-color: #E3E3E3; font-weight: 500">REF ID</th>
                                ...
                                <th style="text-align:center; background-color: #E3E3E3; font-weight: 500">Date</th>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center">
                                        ...
                                        @elseif($header->status == 2)
                                            <span class="badge status-posted m-0" style="width: 100%"> POSTED </span>
                                        ...
                                        @elseif($header->status == 1008)
                                            <span class="badge status-fully-approved m-0" style="width: 100%"> DESIGNATION </span>
                                            ...
                                            <span class="badge status-partially-approved m-0" style="width: 100%; color: black;"> ASGN/DSGNTE </span>
                                    ...
                                    <td style="text-align: center" id="actions">
                                        <a class="btn btn-secondary"  data-bs-toggle="modal" data-bs-target="#requestDetailModal" data-bs-toggle="tooltip" title="View" data-request-id="{{ $header->id }}" style="vertical-align: center; text-align: center"><i class="fa-solid fa-eye"></i></a>
                                      ...
                                      <a  href="{{ route('process.request', ['id' => $header->id]) }}" class="btn btn-primary" data-bs-toggle="tooltip" title="Designate Driver/Vehicle" style="vertical-align: center; text-align: center"><i class="fa-solid fa-gears"></i></a>
                                            @endif
                                            @if($header->status ==1009)
                                        <a class="btn btn-success btn-update-request" class="btn btn-warning"  data-id="{{ $header->id }}" data-status="1010" data-bs-toggle="tooltip" title="Close" style="white-space: nowrap;"><i class="fa-solid fa-ban"></i> 
                                        @endif
                                         @if($header->status ==1010)
                                        <a class="btn btn-success btn-update-request" class="btn btn-warning"  data-id="{{ $header->id }}" data-status="4" data-bs-toggle="tooltip" title="Return" style="white-space: nowrap;"><i class="fa-solid fa-exchange"></i> 
                                        @endif
                                            <!-- <a class="btn" style="opacity: 0; cursor: default"><i class="fa-solid fa-pen-to-square" ></i></a> -->
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    <script>
        ...
        $(document).ready(function (){
            ...
            filterBtn.on('click', function () {
                ...
                console.log("Vehicle:", $('#filter-requested-vehicle').val());
                // alert(dateValue);
...
async function markDeparted() {
   const res = await fetch('{{env('APP_URL')}}/api/run-minute-job', {
     method: 'POST',

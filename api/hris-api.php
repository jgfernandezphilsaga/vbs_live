...
if(isset($_GET['emp'])){
	...
	for($i=0; $rowempdata = $empdatasql->fetch(); $i++){   
		...
		$result .= "<option value='".$rowempdata['EmpID']." - ".$rowempdata['FullName']."'>".$rowempdata['EmpID']." - ".$rowempdata['FullName']."<span style='display:none;'>=".$rowempdata['DeptDesc']."</span></a></option>";
	}
	// echo "xssss<select>".$result."</select>";
	echo json_encode('tset');
/*
// TEMPORARY BUIHIONN GG

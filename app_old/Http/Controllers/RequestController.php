   ...
   public function get_employees(Request $request)
{
    ...
    ->table('driver-monitoring.dbo.driver_details as t1')
    ->leftJoin('vbs_db_new.dbo.driver_vehicle_details as t2', function($join) {
        $join->on(
            DB::raw("t1.employee_id COLLATE Latin1_General_CI_AS"),
            '=',
            DB::raw("t2.driver_details COLLATE Latin1_General_CI_AS")
        );
    })
    ->select('t1.employee_id', 't1.last_name', 't1.first_name')
    ->whereNull('t2.driver_details')
    ->whereNull('t2.returned')
    ->whereNull('t2.returned_date');
    ...
    public function get_vehicles(Request $request, $id)
    {
        $term = $request->term;
            ...
            $allowedTypes = DB::connection('sqlsrv')
            ...
            ->leftJoin('vbs_db_new.dbo.driver_vehicle_details as t2', function($join) {
                $join->on(DB::raw("[t1].[PLATE No#] COLLATE Latin1_General_CI_AS"), '=', DB::raw("t2.vehicle_details COLLATE Latin1_General_CI_AS"));
            })
            ->select('t1.MODEL', 't1.EQUIPMENT', DB::raw('[t1].[PLATE No#] as PLATE_NO'))
            ->whereNull('t2.vehicle_details')
            ->whereNull('t2.returned')
            ->whereNull('t2.returned_date')
            ->whereIn('t1.EQUIPMENT', $allowedTypes);
...
$userId = session('user_id');
    ...
    if (session('user_role') === 'gsd_dispatcher') {
            ...
            ->join('users', 'users.id', '=', 'request_headers.user_id')
            ->select('request_headers.*', 'users.full_name')
            ->whereIn('request_headers.status', [1007, 1011,1009, 1010,4]);
        ...
        $approved_closed = DB::connection('sqlsrv')->table('request_headers')->where('status', '1010')->count();
        $completed = DB::connection('sqlsrv')->table('request_headers')->where('status', '4')->count();
        ...
        return view('admin.dashboard.dispatch', compact('request_headers','vehicle_types','departments','dispatch', 'pending', 'approved_open','approved_closed'));
        }
    ...
    public function store(Request $request)
    {
        ...
        $requested_vehicle_qty = $request->vehicle_qty;
        ...
        foreach ($requested_vehicle as $index => $type) {
         $values[] = [
        'header_id' => $new_request->id,
        'vehicle_type' => $type,
        'qty' => $requested_vehicle_qty[$index]];
        }
        DB::connection('sqlsrv')->table("vehicle_types_tbl")->insert($values);  
        ...
        for($index = 0; $index < $no_of_rows; $index++)
        {
            ...
            $detail = RequestDetail::create([
                ...
                'end_time' => Carbon::createFromFormat('Y-m-d\TH:i', $dateend[$index]),
        ...
        Session::flash('success', 'Successfully submitted request!');
        return response()->json(['route' => env('APP_URL')]);
    }
    ...
    public function show(string $id)
    {
        ...
        $dispatcher = User::where('id', $header->gsd_dispatcher_id)->pluck('full_name')->first();
            ...
            $rawDriverStrings = DB::connection('sqlsrv2')
                ->table('vbs_db_new.dbo.driver_vehicle_details')
                ->where('header_id', $id)
                ->pluck('driver_details');

 
            $allEmployeeIds = [];

            foreach ($rawDriverStrings as $driverString) {
                $ids = array_map('trim', explode(',', $driverString));
                $allEmployeeIds = array_merge($allEmployeeIds, $ids);
            }
            ...
            $driver_details = DB::connection('sqlsrv2')
                ->table('driver-monitoring.dbo.driver_details')
                ->select('employee_id', 'last_name', 'first_name')
                ->whereIn('employee_id', $allEmployeeIds)
                ->get();

        
            $rawVehicleStrings = DB::connection('sqlsrv2')
                ->table('vbs_db_new.dbo.driver_vehicle_details')
                ->where('header_id', $id)
            ...
            foreach ($rawVehicleStrings as $vehicleString) {
                $ids = array_map('trim', explode(',', $vehicleString));
                $allvehiclePlates = array_merge($allvehiclePlates, $ids);
            }

            $allVehiclePlates = array_unique($allvehiclePlates);

           
            $vehicle_details = DB::connection('sqlsrv1')
                ->table('vms_db.dbo.masters')
                ->select('MODEL', DB::raw('[PLATE No#] as PLATE_NO'))
                ->whereIn(DB::raw('[PLATE No#]'), $allVehiclePlates)
                ->get();

        
        $vehicle_types = DB::connection('sqlsrv')
        ->table('vehicle_types_tbl')
        ->where('header_id',$id)
        ->get();     
    ...
    public function update(Request $request, string $id)
    {
        ...
        $is_nightdrive = $request->is_nightdrive;
        $passengers = $request->passengers != null ? $request->passengers : [[]];
        $requested_vehicle = $request->input('requested_vehicle', []); 
        $all_types = $request->input('all_vehicle_types', []); 
        $requested_vehicle_qty = $request->input('vehicle_qty', []);
        ...
        $insert = [];
                    ...
                    foreach ($requested_vehicle as $type) {
                        $qty = $requested_vehicle_qty[$type] ?? 0;
                        $insert[] = [
                            'header_id' => $header_id,
                            'vehicle_type' => $type,
                            'qty' => $qty,
                    ...
                    if (!empty($insert)) {
                        DB::connection('sqlsrv')->table('vehicle_types_tbl')->insert($insert);
                    }
        ...
        for ($detail_index = 0; $detail_index < count($detail_map); $detail_index++) {
            ...
            if(isset($existing_details_ids[$detail_index])) 
            {
                RequestDetail::where('id', $existing_details_ids[$detail_index])->update([
                    ...
                    'end_time' => Carbon::createFromFormat('Y-m-d\TH:i',$detail['end_time']),
    ...
    public function update_status(Request $request)
    {
        ...
        switch ($status) {
            ...
            case 1011:
            $host = 'mlsvrvhcbooking';
            $port = 80;
            $path = '/VBS/public/api/wfs/vbs_post.php';
    ...
    $url ='http://mlsvrvhcbooking/VBS/public/api/wfs/vbs_update.php';
        $params = [
            'transid'        => $transid,
        ];
